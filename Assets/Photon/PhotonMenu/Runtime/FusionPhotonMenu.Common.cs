...
namespace Fusion.Menu {
  ...
  public partial class PhotonMenuGraphicsSettings {
    ...
    /// </summary>
    public virtual List<int> CreateFramerateOptions => PossibleFramerates.Where(f => f <= GetRefreshRate(Screen.currentResolution)).ToList();
    ...
    private int GetCurrentResolutionIndex() {
      ...
      int defaultRefreshRate = GetRefreshRate(resolutions[^1]);

      for (int i = 0; i < resolutions.Length; i++) {
        var resolution = resolutions[i];

        if (resolution.width == currentWidth && resolution.height == currentHeight && GetRefreshRate(resolution) == defaultRefreshRate)
          return i;
      }
	...
	private static int GetRefreshRate(UnityEngine.Resolution resolution)
	{
#if UNITY_2022_3_OR_NEWER
		return UnityEngine.Mathf.RoundToInt((float)resolution.refreshRateRatio.value);
#else
