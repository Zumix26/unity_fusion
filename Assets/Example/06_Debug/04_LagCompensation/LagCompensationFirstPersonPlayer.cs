	...
	public sealed class LagCompensationFirstPersonPlayer : FirstPersonExpertPlayer
	{
		...
		protected override void ProcessInputAfterFixedMovement()
		{
			...
			if (Input.WasActivated(EExpertInputAction.LMB) == true && Runner.IsForward == true)
			{
				...
				if (KCC.ResolveRenderPositionAndLookRotation(CameraHandle, Input.FixedInput.RenderAlpha, out Vector3 renderCameraPosition, out Quaternion renderLookRotation) == true)
				{
		...
		protected override void ProcessInputAfterRenderMovement()
		{
			...
			if (Input.WasActivated(EExpertInputAction.LMB) == true)
			{
				...
				if (KCC.ResolveRenderPositionAndLookRotation(CameraHandle, Input.RenderInput.RenderAlpha, out Vector3 renderCameraPosition, out Quaternion renderLookRotation) == true)
				{
