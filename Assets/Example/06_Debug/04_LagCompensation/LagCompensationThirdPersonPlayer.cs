	...
	public sealed class LagCompensationThirdPersonPlayer : ThirdPersonExpertPlayer
	{
		...
		protected override void ProcessInputAfterFixedMovement()
		{
			...
			if (Input.WasActivated(EExpertInputAction.LMB) == true && Runner.IsForward == true)
			{
				...
				if (_cameraHandleSampler.ResolveRenderPositionAndRotation(KCC, Input.FixedInput.RenderAlpha, out Vector3 renderCameraPosition, out Quaternion renderCameraRotation) == true)
				{
		...
		protected override void ProcessInputAfterRenderMovement()
		{
			...
			if (Input.WasActivated(EExpertInputAction.LMB) == true)
			{
				...
				if (_cameraHandleSampler.ResolveRenderPositionAndRotation(KCC, Input.RenderInput.RenderAlpha, out Vector3 renderCameraPosition, out Quaternion renderCameraRotation) == true)
				{
